...
SELECT brand ,model,cars.price from cars;
--#2 Display quantity of all cars  #2 Таблицадан бардык машиналардын санын чыгарыныз
SELECT COUNT(*)AS total_cars FROM cars;

--#3 Display all cars where brand is Hyundai  #3 Hyundai брендиндеги бардык машиналарды чыгарыныз
SELECT *FROM  cars WHERE brand= 'Hyundai';

--#4 Display all Red and Blue cars #4 Бардык кызыл жана кок машиналарды чыгарыныз
SELECT  * from cars where color in ('Red','Blue');

--#5 Display all cars where issued year between 2000 and 2010 #5 Бардык 2000-жылдан 2010-жылга чейинки чыгарылган машиналарды чыгарыныз
...
SELECT AVG(year_of_issue)AS avg_year from cars;
...
SELECT  DISTINCT  brand from cars where LENGTH(brand)=5;

--#12 Бренди Mercedes-Benz болгон машиналардын жалпы суммасын чыгарыныз
SELECT SUM(price) AS total_price FROM cars WHERE brand = 'Mercedes-Benz';
...
SELECT * FROM cars WHERE UPPER(brand) <> 'TOYOTA';
...
SELECT * FROM cars ORDER BY year_of_issue DESC OFFSET 4   LIMIT 11;
...
SELECT color, COUNT(*) AS total FROM cars GROUP BY color ORDER BY total DESC LIMIT 1;